...
def save_masked_videos_grid(videos: torch.Tensor, masks: torch.Tensor, path: str, rescale=False, n_rows=6, fps=8):
    videos = rearrange(videos, "b c t h w -> t b c h w")
    masks = rearrange(masks, "b t h w -> t b h w") 
    outputs = []
    
    for x, mask in zip(videos, masks):
        x = torchvision.utils.make_grid(x, nrow=n_rows) 
        mask = torchvision.utils.make_grid(mask.unsqueeze(1), nrow=n_rows)
        ...
        if rescale:
            x = (x + 1.0) / 2.0 # [-1,1] -> [0,1]
        x = (x * 255).astype(np.uint8)  # uint8

        # mask = mask.squeeze(0).numpy()  # mask to numpy
        mask_rgb = np.stack([mask * 255, np.zeros_like(mask), np.zeros_like(mask)], axis=-1)  # red mask
        ...
        alpha = 0.5  # opacity
        x = np.clip(x * (1 - alpha) + mask_rgb * alpha, 0, 255).astype(np.uint8)  # Overlay mask
...
def load_weights(
    ...
    if motion_module_path != "":
        ...
        motion_module_state_dict = {key.replace("module.", "", 1): value for key, value in motion_module_state_dict.items()} ## modify this
    ...
    if dreambooth_model_path != "":
        ...
        if dreambooth_model_path.endswith(".safetensors"):
            ...
            dreambooth_state_dict = dreambooth_state_dict["state_dict"] if "state_dict" in dreambooth_state_dict else dreambooth_state_dict
            dreambooth_state_dict = {key.replace("module.", "", 1): value for key, value in dreambooth_state_dict.items()} ## modify this

            # print(list(dreambooth_state_dict.keys()))

            m, u = animation_pipeline.unet.load_state_dict(dreambooth_state_dict, strict=False)
