...
class Something_v2_seg_v2(Dataset):
    def __init__(
            self,
        ...
        self.pixel_transforms = transforms.Compose([
            ...
            transforms.Normalize(mean=[0.5, 0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5, 0.5], inplace=True),
        ])
    ...
    def get_batch(self, idx):
        ...
        pixel_values_2, _ = self.get_pixel_values(self.video_seg_folder, videoid + "_highlighted", start_idx, binary=True)

        pixel_values = torch.concatenate((pixel_values_1, pixel_values_2), dim = 1)
        # print(pixel_values.shape)
...
if __name__ == "__main__":
    from animatediff.utils.util import save_videos_grid, save_masked_videos_grid


    dataset = Something_v2_seg_v2(
    ...
    for idx, batch in enumerate(dataloader):
        ...
        pixel_value, mask = pixel_value[:3, ...], pixel_value[3, ...]
        pixel_value = pixel_value[None, ...]
        mask = mask[None, ...]
    
        
        text = batch["text"][0]
        output_dir = '/home/dungnt206/workspace/code/AnimateDiff/outputs/20_percent/v3/bins'
        print(pixel_value.shape)
        print(text)
        save_masked_videos_grid(pixel_value, mask, f"{output_dir}/sanity_check/{'-'.join(text.replace('/', '').split()[:10]) if not text == '' else f'{idx}'}.gif", rescale=True)
